...
function showTalentDetails(talent) {
  ...
  detailsContainer.innerHTML = `
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="detail-label mb-2">面试记录</div>
                <div class="interview-record-container">
                    <textarea id="interviewRecordText" class="form-control mb-2" rows="5" placeholder="请输入面试记录...">${escapeHtml(talent.interviewRecord || "")}</textarea>
                    <button id="saveInterviewRecordBtn" class="btn btn-primary" data-phone="${talent.phone}">保存面试记录</button>
                </div>
            </div>
        </div>
    `;
